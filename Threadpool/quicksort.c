...
static TASK(int, quicksort, QuicksortArg);

static int quicksort(QuicksortArg arg) {
    if (arg.len <= 1)
        return 0;

    size_t mid = partition(arg.list, arg.len);
    quicksort_fut *a = quicksortAsync(
        (quicksort_fut[]){quicksortFuture((QuicksortArg){arg.list, mid})});
    quicksort_fut *b = quicksortAsync((quicksort_fut[]){quicksortFuture(
        (QuicksortArg){arg.list + mid + 1, arg.len - mid - 1})});
...
static struct timespec time_diff(struct timespec t1, struct timespec t2) {
    struct timespec result = {.tv_sec = t2.tv_sec - t1.tv_sec,
...
int main(int argc, const char *argv[]) {
    ...
    quicksort((QuicksortArg){list, list_len});
